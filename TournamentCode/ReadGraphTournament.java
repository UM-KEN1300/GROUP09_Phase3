...
public class ReadGraphTournament
{
    ...
    System.out.println("NEW BEST LOWER BOUND = " + lowerBound);
		...
		if(partialColoring.length == 0) {
			...
			System.out.println("CHROMATIC NUMBER = " + adj_matrix.length);
			stop(programStart);
		}
		//Run Backtracking...
		start = System.nanoTime();
		...
		eval(lowerBound, backtrackingResult, programStart);
		//Run DSatur...
		start = System.nanoTime();
		...
		for(int i = lowerBound; i <= bestUpperBound; i++) {
			...
			if(tmp == 1) {
				System.out.println("CHROMATIC NUMBER = " + i);
				stop(programStart);
			}
	...
	*/
	private static void eval(int lowerBound, int upperBound, long programStart) {
      ...
      System.out.println("NEW BEST UPPER BOUND = " + bestUpperBound);
		}
	}
	/**
